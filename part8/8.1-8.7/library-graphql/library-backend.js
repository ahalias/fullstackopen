...
const resolvers = {
  Query: {
    authorCount: (root, args) => {
      ...
      return authors.filter(author => author.name === root.author).length
    },
    allAuthors: () => authors,
    ...
    bookCount: (root, {author}) => {
      ...
      return books.filter(book => book.author === author).length
    },
    allBooks: (root, {author, genre}) => {
      if (author && genre) {
        return books.filter(book => book.genres.includes(genre) && book.author === author)
      } else if (author) {
  ...
  Mutation: {
    addBook: (root, args) => {
      ...
      books.push(book);
      const bookToAdd = books.find((book) => book.title === args.title);
      try {
